...
from Basic_Algorithms.fcm_code import fcm, validity2
...
class it2fcm:
    ...
    def compute_u(self, mode): 
        if mode==0:
            distance=np.linalg.norm(self.x[:, np.newaxis, :]-self.v, axis=2)
        else:
            distance=np.linalg.norm(self.x[:, np.newaxis, :]-self.v, axis=2)

            distance=np.linalg.norm(self.x[:, np.newaxis, :]-self.v, axis=2)*(1-0.8*np.e**(-self.SI_hard))
    ...
    def compute_distance(self, data:np.ndarray, alpha):
        distance=cdist(data, data)
        distance[np.where(distance>alpha)]=0
        used_index=np.where(distance!=0)
        distance[used_index]=distance[used_index]**-1
        self.distance = distance

    def compute_SI(self, uu:np.ndarray):
        distance2=self.distance.T[:, :, np.newaxis]
        return np.sum(distance2*uu.T, axis=1)/np.sum(distance2, axis=1)
    ...
    def run(self):
        self.compute_distance(self.x, 1)
        for i in range(self.max_iter):
            ...
            for j in range(self.number_of_centroid): 
                ...
                real_u_upper.append(km_for_upper[1])
                real_u_lower.append(km_for_lower[1])
            upper_centroid=np.array(upper_centroid)
            lower_centroid=np.array(lower_centroid)
            real_u_upper=np.stack(real_u_upper, axis=0)
            SI_upper=self.compute_SI(real_u_upper)
            real_u_lower=np.stack(real_u_lower, axis=0)
            SI_lower=self.compute_SI(real_u_lower)
            self.SI_hard=(SI_lower+SI_upper)/2
            print(self.SI_hard)
...
if __name__ == '__main__':
    ...
    fcmm=fcm(eps=1e-2, max_iter=1000, m=2)
    u, v, i=fcmm.start(x, 3, init_centroid)
    print("\n\nChi so FCM")
    validity2(data=x, clustter=v, membership=u, target=y)
    print("\n\n")
