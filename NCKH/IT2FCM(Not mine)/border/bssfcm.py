import time
import numpy as np
from f1.clustering.utility import extract_labels, division_by_zero
from bfcm import DbFcm

class DbssFcm(DbFcm):
    def __init__(self, n_clusters: int, m: float = 2, epsilon: float = 1e-5, max_iter: int = 10000,
                 index: int = 0, metric: str = 'euclidean', no_label: int = -1):
        ...
        self.labeled_bar = None

    def init_membership_bar(self, n_samples: int, labeled: np.ndarray = None):
        self.membership_bar = np.zeros((n_samples, self._n_clusters))
        self.labeled_bar = np.zeros((n_samples, 1))  # L (Nx1)
        if labeled is not None:
            for i, label in enumerate(labeled):
                if label != self.__no_label:
                    self.membership_bar[i, label] = 1
                    self.labeled_bar[i] = 1
    ...
    def calculate_membership(self, distances: np.ndarray) -> np.ndarray:
        sum_membership_bar = np.sum(self.membership_bar, axis=1, keepdims=True)
        tu = 1 - sum_membership_bar
        _d = distances[:, :, None] * ((1 / division_by_zero(distances))[:, None, :])
        power = 2 / (self._m - 1)
        mau = (_d ** power).sum(axis=2)
        return self.membership_bar + tu / mau

    def fit(self, data: np.ndarray, labeled: np.ndarray, testid: int = 0) -> tuple:
        self.labeled = labeled
        self.init_membership_bar(n_samples=len(data), labeled=labeled)
        return super().fit(data=data, testid=testid)
...
if __name__ == '__main__':
    ...
    labeleds = remove_label_for_semi_supervised(labels=labels, labeled_percentage=LABELED_RATIOS, seed=42, no_label=NO_LABEL)
    ...
    bssfcm = DbssFcm(n_clusters=C, m=M, epsilon=EPSILON, max_iter=MAXITER,
                 index=0, metric=METRIC, no_label=NO_LABEL)
    
    bssfcm.fit(data=X2, labeled=labeleds)

    print_info(title=ALG, X=X2, U=bssfcm.membership, V=bssfcm.centroids, y_true=labels, process_time=bssfcm.process_time, step=bssfcm.step)
    
# python nckh/f1/clustering/border/bssfcm.py
