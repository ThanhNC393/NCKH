...
from f1.clustering.cmeans.cfcm import Dcfcm
...
class DbCFC(Dcfcm):
    ...
    def phase1(self, datas: list, seed: int = None) -> list:
        ...
        for i, data in enumerate(datas):
            ds = DbFcm(n_clusters=self._n_clusters, epsilon=self._epsilon, max_iter=self._max_iter, index=i, metric=self._metric)
            ds.fit(data=data)
            ds.exited = False
            self._ds.append(ds)
...
if __name__ == '__main__':
    ...
    bcfcm = DbCFC(n_clusters=C, n_sites=P, epsilon=EPSILON, max_iter=MAXITER, metric=METRIC)
    
    bcfcm.phase1(datas=datas, seed=SEED)
    print('#BCFC ----------------------------------------')
    for i, dsi in enumerate(bcfcm.datasites):
...
# python nckh/f1/clustering/border/bcfc.py
