...
import numpy as np
import math
from f1.clustering.utility import norm_distances, distance_pdist, distance_cdist, extract_labels, not_division_by_zero, optimized_pred_labels

# FB +
def fb_score(data: np.ndarray, membership: np.ndarray, centroids: np.ndarray, metric: str = 'euclidean') -> float:
    labels = extract_labels(membership)
    C = np.unique(labels, return_counts=True)
    n_cluster = len(C[0])
    number_data_of_cluster = C[1]

    result = 0
    for i in range(n_cluster):
        max = 1e-10
        element_1 = (1 / number_data_of_cluster[i]) * (np.sum(membership[labels == i] * distance_cdist(data[labels == i], centroids[i][None, :], metric=metric)))
        for j in range(n_cluster):
            if i != j:
                element_2 = (1 / number_data_of_cluster[j]) * (np.sum(membership[labels == j] * distance_cdist(data[labels == j], centroids[j][None, :], metric=metric)))
                denominator = np.linalg.norm(centroids[i] - centroids[j])
                result = (element_1 + element_2) / not_division_by_zero(denominator)
                if result > max:
                    max = result
        result += max
    return result / n_cluster
...
# AC + (tối ưu)
def accuracy_score_optimize(y_true: np.ndarray, y_pred: np.ndarray) -> float:
    optimized = optimized_pred_labels(y_true, y_pred)
    return accuracy_score(y_true, optimized)
...
def cs(data: np.ndarray, membership: np.ndarray, centroids: np.ndarray, m: float = 2) -> float:
    ...
    return numerator / division_by_zero(N * min_center_dist)
...
# TAU -
def tau(data: np.ndarray, centroids: np.ndarray, typicality: np.ndarray, metric: str = 'euclidean') -> float:
    """
    Tính chỉ số TAU index
    ...
    Return:
    Giá trị của chỉ số TAU index
    """
    _, C = typicality.shape
    labels = extract_labels(typicality)
    clusters = [data[labels == i] for i in range(C)]

    S_iq = np.asanyarray([np.mean([np.linalg.norm(point - centroids[i]) for point in cluster]) for i, cluster in enumerate(clusters)])
    tu = np.sum(np.square(typicality) * np.square(S_iq))
    distance = distance_cdist(data, centroids, metric=metric)
    distance[distance == 0] = math.inf
    mau = len(data) * np.min(np.square(distance))
    return tu / mau


# Trung bình khoảng cách các tâm ABC +
...
# Trung bình tổng khoảng cách điểm tới tâm ADC +
def mean_distance_point_cluster(data: np.ndarray, centroids: np.ndarray, metric: str = 'euclidean') -> float:
    ...
    distances = distance_cdist(data, centroids, metric=metric)
    # Tính tổng khoảng cách của mỗi điểm tới tất cả các tâm cụm
    total_distances = np.sum(distances, axis=1)
    # Tính giá trị trung bình của các tổng khoảng cách
    return np.mean(total_distances)
...
# MSE -
# Khoảng cách từ các điểm dữ liệu đến tâm cụm tương ứng
def mse(data: np.ndarray, centroids: np.ndarray, membership: np.ndarray) -> float:
    """
    Tính chỉ số MSE cho kết quả phân cụm

    Parameters:
    data: np.array - dữ liệu (NxD)
    centroids: np.array - các tâm cụm (CxD)
    membership: np.array - ma trận độ thuộc (NxC)

    Returns:
    float: giá trị MSE
    """
    n_samples, result = len(data), 0
    labels = extract_labels(membership)
    # Tính tổng bình phương khoảng cách từ mỗi điểm đến tâm cụm tương ứng
    for i in range(n_samples):
        cluster_center = centroids[labels[i]]
        result += np.sum((data[i] - cluster_center) ** 2)

    # Chia cho số lượng mẫu
    return result / n_samples
    # labels = extract_labels(membership)
    # from sklearn.metrics import mean_squared_error
    # return mean_squared_error(data, centroids[labels])

# ================================
def optimized_pred_labes(labels: np.ndarray, pred_labels: np.ndarray) -> np.ndarray:
...
# AC + (optimized)
def accuracy_score_optimize(y_true: np.ndarray, y_pred: np.ndarray) -> float:
    optimized = optimized_pred_labes(labels=y_true, pred_labels=y_pred)
    return accuracy_score(y_true, optimized)

def accuracy_for_data_non_label(y_true: np.ndarray, y_pred: np.ndarray, labeled: np.ndarray, no_label: int = -1) -> float:
    y_true = y_true[labeled == no_label]
    y_pred = y_pred[labeled == no_label]
