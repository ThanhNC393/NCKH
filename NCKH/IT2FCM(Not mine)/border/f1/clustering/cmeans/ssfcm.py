...
class Dssfcm(Dfcm):
    def __init__(self, n_clusters: int, m: float = 2, epsilon: float = 1e-5, max_iter: int = 10000, index: int = 0, metric: str = 'euclidean', no_label: int = -1):
    ...
    def fit(self, data: np.ndarray, labeled: np.ndarray, seed: int = 0, no_label: int = -1) -> tuple:
        ...
        _n_samples = len(data)
        self.labeled_bar = np.zeros((_n_samples, 1))
...
if __name__ == "__main__":
    ...
    from f1.clustering.dataset import fetch_data_from_local, TEST_CASES, LabelEncoder, remove_label_for_semi_supervised
    ...
    ROUND_FLOAT = 3
    MAX_ITER = 1000
    DATA_ID = 53  # 14: Breast Cancer, 53: Iris, 80: Digits, 109: Wine, 236: Seeds, 602: DryBean
    EPSILON = 1e-5
    LABELED_RATIOS = 0.1
    SEED = 42
    M = 2
    ...
    if DATA_ID in TEST_CASES:
        ...
        _dt = fetch_data_from_local(DATA_ID, folder='/home/manhnv343/Documents/gdrive/core/CoreFCM/dataset')
        if not _dt:
        ...
        X = _dt['X']
        Y = remove_label_for_semi_supervised(labels=labels, labeled_percentage=LABELED_RATIOS, seed=SEED, no_label=NO_LABELED)
