...
import time
import numpy as np
from IT2FCM.border.f1.clustering.utility import distance_cdist, extract_labels, extract_clusters
...
class Dfcm():
    ...
    def predict(self, new_data: np.ndarray, m: float = 2) -> np.ndarray:
        _new_u = self._update_membership(new_data, self.centroids, m=m)
    ...
    # CHECK EXIT BEGIN ================================================
    def _update_membership(self, data: np.ndarray, centroids: np.ndarray, m: float = 2) -> np.ndarray:
    ...
    def __fit_with_centroid(self, init_v: np.ndarray = None, seed: int = 0, device: str = 'CPU'):
        ...
        for _step in range(self._max_iter):
            ...
            self.membership = self._update_membership(self.local_data, old_v, m=self._m)
    ...
    def __fit_with_membership(self, init_u: np.ndarray = None, seed: int = 0, device: str = 'CPU'):
        ...
        for _step in range(self._max_iter):
            ...
            self.membership = self._update_membership(self.local_data, self.centroids, m=self._m)
