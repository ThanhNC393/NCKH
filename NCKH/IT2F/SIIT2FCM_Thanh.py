...
class it2fcm:
    ...
    def compute_u(self, mode): 
        if mode==0:
            ...
            distance=np.linalg.norm(self.x[:, np.newaxis, :]-self.v, axis=2)*(1-0.95*np.e**(-self.SI_hard))
    ...
    def compute_SI(self, u:np.ndarray):
        distance2=self.distance.T[:, :, np.newaxis]
        tmp=np.sum(distance2, axis=1)
        tmp[np.where(tmp==0)]=1
        return np.sum(distance2*u.T, axis=1)/tmp
    ...
    def run(self):
        self.compute_distance(self.x, 0.1)
...
if __name__ == '__main__':
    ...
    run_it2fcm=it2fcm(max_iter=1000, m1=2, m2=4, x=x, init_centroid=init_centroid, number_of_centroid=3)
    real_u=run_it2fcm.run()

    print('Chi so SIIT2FCM')
