...
import pandas as pd
import math
from utility import norm_distances, distance_pdist, distance_cdist, round_float
...
def print_metrics(X: np.ndarray, y:list, U: np.ndarray, V: np.ndarray):

    y_pred = np.argmax(U, axis=1)
    y_true, _ = pd.factorize(y)
    print('DI:', dunn(X, y_pred))
    print('DB:', davies_bouldin(X, y_pred))
    print('PC:', partition_coefficient(U))
    print('PE:' , partition_entropy(U))
    print('CE:', classification_entropy(U))
    print('CH:', calinski_harabasz(X, y_pred))
    print('SI:', silhouette(X, y_pred))
    print('FHV:', hypervolume(U))
    print('CS:', cs(X, U, V))
    print('S:', separation(X, U, V))
    # print('XB:',Xie_Benie(X, V, U))
...
def print_info(title: str, X: np.ndarray, U: np.ndarray, V: np.ndarray, process_time: float, M:float = 2, step: int = 0, split: str = SPLIT) -> str:
    labels = np.argmax(U, axis=1)  # Giai mo
    ...
    kqdg = [str(item) for item in kqdg]
